...
const skills = [
  { name: "HTML5 95%", icon: <FaHtml5 />, color: "text-orange-600" },
  ...
  return (
    <section id="skills" className="py-24 px-6 bg-gradient-to-b  from-blue-100 via-white to-blue-100
      ...
      <div className="max-w-6xl mx-auto text-center" data-aos="fade-right">
        <h2 className="text-4xl sm:text-4xl md:text-5xl font-extrabold font-heading my-4 text-blue-900 dark:text-white text-center md:text-left">
        Tech <span className="text-blue-500 dark:text-cyan-400">Stack</span>
      </h2>

      <p className="my-5 text-sm sm:text-base md:text-lg text-gray-700 dark:text-gray-300 tracking-wide leading-relaxed sm:leading-loose max-w-2xl md:max-w-3xl mx-auto md:mx-0 text-center md:text-left">
        I craft fast, modern UIs using{" "}
        <span className="font-semibold text-blue-500 dark:text-cyan-400">React</span>,{" "}
        <span className="font-semibold text-blue-500 dark:text-cyan-400">Tailwind CSS</span>, and{" "}
        <span className="font-semibold text-blue-500 dark:text-cyan-400">Vite</span>{" "}
        and deploy pixel-perfect. websites with{" "}
        <span className="font-semibold text-blue-500 dark:text-cyan-400">Vercel</span>. 
        Clean code. <br className="hidden sm:block" /> Smooth design. No excuses.
      </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 cursor-pointer">
          {skills.map((skill, index) => (
            ...
            >
              ...
              <h3 className="lg:text-lg text-sm text-gray-900 dark:text-white">{skill.name}</h3>
            </div>
          ))}
        </div>
      </div>
